...
function handleRatingUpdated(e: FormEvent<HTMLFormElement>, id: string){
    ...
    if (target instanceof HTMLElement){
        ...
        if(doc){
            doc.className = "block p-2 m-2 bold font-bold text-white text-xl";
        }
    }
}
...
function getMediaInfo(singleMovie: moviesType, searchType: string){ //return appropriate jsx for movie
    ...
    let title = <h1 className="text-3xl text-white text-center bg-red-950 p-2">{singleMovie["title"]}</h1>;
    let release_date = <p className="pl-2 text-red-400">Release Date: {singleMovie["release_date"]} </p>
    let rateButton = <button className="p-2 m-2 text-white bg-red-700 hover:bg-red-500">Rate this Movie</button>;
    if (searchType === "show"){ // replace differing fields
        title = <h1 className="text-3xl text-white text-center bg-red-950 p-2">{singleMovie["name"]}</h1>;
        release_date = <p className="pl-2 text-red-400">Air Date: {singleMovie["first_air_date"]} </p>;
        rateButton = <button className="p-2 m-2 text-white bg-red-700 hover:bg-red-500">Rate this Show</button>;
    }

    return(
        <div className="w-auto border-black text-white border-2 m-4 bg-red-950">
            {title}
            ...
            {release_date}
            <p className="p-2 text-white">{singleMovie["overview"]}</p>
            <form action={rateMedia} onSubmit={e => handleRatingUpdated(e, singleMovie["id"])}>
                {rateButton}
                <input type="hidden" name="movieID" value={singleMovie["id"]}/>
                <input type="number" name="rating" min="0" max="10" className="m-2 p-2 bg-black text-white inline-block" />
                <p className="inline-block text-white">/ 10</p>
                <p className="hidden text-white" id={singleMovie["id"]}>Rating Updated!</p>
            </form>
        </div>
    )
}
...
export function GetMovieInfo(singleMovie: moviesType){
    ...
    return(
        <div className="w-auto border-black text-white border-2 m-4 bg-red-950">
            <h1 className="text-3xl text-center text-white bg-red-950 p-2">{singleMovie["title"]}</h1>
            <img src={"https://image.tmdb.org/t/p/original/" + singleMovie["poster_path"]} alt="No poster" className=""/>
            <p className="pl-2 pt-2 font-bold text-red-400">Rating: {singleMovie["vote_average"]} / 10</p>
            <p className="pl-2 text-red-400">Release Date: {singleMovie["release_date"]} </p>
            <p className="p-2 text-white">{singleMovie["overview"]}</p>
            <form action={rateMedia} onSubmit={e => handleRatingUpdated(e, singleMovie["id"])}>
                <button className="p-2 m-2 text-white bg-red-700 hover:bg-red-500">Rate this Movie</button>
                <input type="hidden" name="movieID" value={singleMovie["id"]}/>
                <input type="number" name="rating" min="0" max="10" className="m-2 p-2 bg-black text-white inline-block" />
                <p className="inline-block text-white">/ 10</p>
                <p className="hidden text-white" id={singleMovie["id"]}>Rating Updated!</p>
...
export function GetShowInfo(singleShow: moviesType){
    ...
    return(
        <div className="w-auto m-4 text-white bg-red-950">
            <h1 className="text-3xl text-white text-center bg-red-950 p-2">{singleShow["name"]}</h1>
            ...
            <p className="p-2 text-white">{singleShow["overview"]}</p>
            <form action={rateMedia} onSubmit={e => handleRatingUpdated(e, singleShow["id"])}>
                <button className="p-2 m-2 text-white bg-red-700 hover:bg-red-500">Rate this Show</button>
                <input type="hidden" name="movieID" value={singleShow["id"]}/>
                <input type="number" name="rating" min="0" max="10" className="m-2 p-2 bg-black text-white inline-block" />
                <p className="inline-block text-white">/ 10</p>
                <p className="hidden text-white" id={singleShow["id"]}>Rating Updated!</p>
            </form>
        </div>
    )
}